...
4. **用户配置目录** `<app_config_dir>/config.json`
   ...
   - 应用内「导入 SSH 主机」与「生成示例配置」都写到这里

> `examples/config.example.json` **不参与自动查找**（第 13/14 项已移除）——它是给人复制的模板，占位符会通过校验，当成配置加载只会得到连不上的主机。
...
- 若**找不到任何配置**，客户端会读取本机 `~/.ssh/config`：
  - **发现有 `Host` 别名** → 左侧列出「用 `<别名>` 开始」按钮，点一下即生成配置并重新加载。地址/用户名由 ssh 解析，无需手填。
  - **一个都没有**（或 `~/.ssh` 不存在）→ 显示「未找到配置文件」面板并列出上面所有已查找路径，提供：
    - **生成示例配置**：在用户配置目录写入模板（随后自动打开编辑面板，把 `REPLACE_WITH_*` 换成真实值）；
    - **重新加载**：手动重新查找。
- 也可手动把 `config.json` 放到 exe 同级目录（第 3 项）。

> 仅支持本机 `~/.ssh/config` 里**直接写的** `Host` 别名：通配符（`*.example.com`）与 `Host *` 块会被跳过；`Include` 进来的文件不会被展开（但 ssh 连接时照常生效）。
